...
def get_particle_data(file, save_file, num_lines=150, mode='ab'):
...
def boundary_generation(box_save_file):
    ...
    # generate the bottom, without the common line with front and back
    bottom_x = np.arange(-1, 1.05, 0.05)
    for each in bottom_x:
        box_pos.append([each, 0., 0.])
        box_pos.append([each, 0., -0.05])
        box_pos.append([each, 0., 0.05])
        box_normals.extend([[0., 1., 0.]] * 3)

    # generate front and back
    x = np.arange(-1, 1.05, 0.05)
    y = np.arange(0, 4.05, 0.05)
    xv, yv = np.meshgrid(x, y)

    for each_pair in zip(xv.flatten(), yv.flatten()):
        box_pos.append([each_pair[0], each_pair[1], 0.1])
    ...
    # y = np.arange(0.05, 4.05, 0.05)
    # for each in y:
    #     box_pos.append([1., each, 0.])
    #     box_pos.append([1., each, -0.05])
    #     box_pos.append([1., each, 0.05])
    #     box_normals.extend([[-1., 0., 0.]] * 3)
    #
    #     box_pos.append([-1., each, 0.])
    #     box_pos.append([-1., each, -0.05])
    #     box_pos.append([-1., each, 0.05])
    #     box_normals.extend([[1., 0., 0.]] * 3)
    # generate left and right, not align perfectly, with an interlace of 0.025 -> radius
    y1 = np.arange(0.05, 4.05, 0.05)
    for each in y1:
        box_pos.append([1., each, 0.])
    ...
    y2 = np.arange(0.025, 4.025, 0.05)
    for each in y2:
        box_pos.append([1., each, 0.05])
        box_pos.append([1., each, -0.05])
        box_normals.extend([[-1., 0., 0.]] * 2)
...
def boundary_generation_7layer(box_save_file):
    # generate a list in the form of [box_pos_matrix, box_normals_matrix]
    box_pos = []
    box_normals = []
    ...
    for each in bottom_x:
        box_pos.append([each, 0., 0.])
        box_pos.append([each, 0., 0.05])
        box_pos.append([each, 0., 0.1])
        box_pos.append([each, 0., -0.05])
        box_pos.append([each, 0., -0.1])
        box_normals.extend([[0., 1., 0.]] * 5)
    ...
    for each_pair in zip(xv.flatten(), yv.flatten()):
        box_pos.append([each_pair[0], each_pair[1], 0.15])
        box_normals.append([0., 0., -1.])
    ...
    with open('box_train-7layer.pkl', 'wb') as f:
        pickle.dump([box_pos, box_normals], f)
...
def data_normalization_20_80(origin_file, save_file):
    # as for initialization, make x, z in range of [-0.5, 0.5], y in range of [0, 1.5]
    # core idea and goal is to make the whole range as pos in [-1, 1], vel in [-4, 4]
    x_target_l = -1
    x_target_up = 1
    ...
    for each_step in dataset:   # [pos_matrix, vel_matrix, label_matrix(t+1), label_matrix(t+2)]
        # normalize pos_matrix
        each_step[0][:, 0] = x_target_l + 2 / 60 * (each_step[0][:, 0] - 20)
        each_step[0][:, 1] = y_target_l + 4 / 60 * (each_step[0][:, 1] - 20)

        # normalize label_matrix
        each_step[2][:, 0] = x_target_l + 2 / 60 * (each_step[2][:, 0] - 20)
        each_step[2][:, 1] = y_target_l + 4 / 60 * (each_step[2][:, 1] - 20)

        each_step[3][:, 0] = x_target_l + 2 / 60 * (each_step[3][:, 0] - 20)
        each_step[3][:, 1] = y_target_l + 4 / 60 * (each_step[3][:, 1] - 20)

        # normalize vel_matrix
        each_step[1][:, 0] = 2 / 60 * (each_step[1][:, 0])
        each_step[1][:, 1] = 4 / 60 * (each_step[1][:, 1])
...
def check_dataset(file_path='30-500dataset'):
    ...
    for file in files:
        ...
        print('lines: ', len(open(data_path, 'r').readlines()))
...
# boundary_generation_7layer('box_train-7layer.pkl')
# get_particle_data('30-500dataset/output1.txt', 'fluid_train_temp.pkl', mode='wb')
# get_particle_data('30-500dataset/output2.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output3.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output4.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output5.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output6.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output7.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output8.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output9.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output10.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output11.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output12.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output13.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output14.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output15.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output16.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output17.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output18.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output19.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output20.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output21.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output22.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output23.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output24.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output25.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output26.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output27.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output28.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output29.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output30.txt', 'fluid_train_temp.pkl')
# get_particle_data('30-500dataset/output31.txt', 'fluid_eval_temp.pkl', mode='wb')
#
# data_normalization_20_80('fluid_train_temp.pkl', 'fluid_train_20_80.pkl')
# data_normalization_20_80('fluid_eval_temp.pkl', 'fluid_eval_20_80.pkl')


# check_dataset()
#
# dataset = []
# with open('fluid_train_20_80.pkl', 'rb') as f:
#     while True:
#         try:
#             data = pickle.load(f)
#             print(len(data))
