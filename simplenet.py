...
class MyParticleNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.layer_channels = [32, 3] #[32, 64, 64, 3]
        ...
        setattr(self, 'dense{0}'.format(1), dense)
        conv = Conv(name='conv{0}'.format(1),
                    in_channels=in_ch,
                    filters=out_ch,
                    activation=None)
        setattr(self, 'conv{0}'.format(1), conv)
    ...
    def compute_correction(self,
        ...
        filter_extent = torch.tensor(self.filter_extent) # TODO: radius * scale * 6
        ...
        # ml3d.ops.reduce_subarrays_sum: [1, 2, 3, 4] [0, 2, 2, 4] the result will be [1+2, 0, 3+4]
    ...
    def forward(self, inputs, fixed_radius_search_hash_table=None):
        ...
        """
        pos, vel, feat, box, box_feats = inputs

        pos2, vel2 = self.integrate_pos_vel(pos, vel)
        pos_correction = self.compute_correction(
            pos2, vel2, box, box_feats, fixed_radius_search_hash_table)
    ...
    #     pos = np.zeros(shape=(1, 2), dtype=np.float32)
    #     vel = np.zeros(shape=(1, 2), dtype=np.float32)
    #     if feats_shape is None:
    #         feats = None
    #     else:
    #         feats = np.zeros(shape=feats_shape, dtype=np.float32)
    #     box = np.zeros(shape=(1, 2), dtype=np.float32)
    #     box_feats = np.zeros(shape=(1, 2), dtype=np.float32)
    #
    #     _ = self.__call__((pos, vel, feats, box, box_feats))

# if __name__ == '__main__':
#     # pos = torch.tensor([[1., 2., 0.],[1.1, 1.2, 0.],[1.3,1.4, 0.]])
#     # vel = torch.tensor([[1., 2., 0.],[1.1, 1.2, 0.],[1.3,1.4, 0.]])
#     # pos1 = torch.tensor([[1.1, 2.1, 0.],[1.2, 1.3, 0.],[1.4,1.5, 0.]])
#     # box = torch.tensor([[5.,5., 0.]])
#     # box_feat = torch.tensor([[0.,0., 0.]])
#     # inputs = [pos, vel, pos1, box, box_feat]
#     dataset = MyDataset("output.txt", 32).dataset
#     data = np.asarray(np.array_split(dataset, 2))
#     batch = data[0]
#     inputs = ([
#         torch.tensor(batch[0][0]), torch.tensor(batch[0][1]), torch.tensor(batch[0][2]),
#         torch.tensor(batch[0][3]), torch.tensor(batch[0][4])
#     ])
